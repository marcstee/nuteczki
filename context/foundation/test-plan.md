...
|---|------------|------------------|----------------|------------|--------|----------------|
| 1 | Bootstrap + exercise integrity | Stand up the runner; prove every generated exercise is winnable and musically correct (no DB needed) | #1, #2 | unit | change opened | context/changes/testing-bootstrap-exercise-integrity/ |
| 2 | Session-boundary regression net | Lock summary correctness and session persistence against the real schema, however composition churns | #3, #4 | unit + integration | not started | — |
...
- TBD — see §3 Phase 2 (session persistence verified against the real local Supabase schema; no schema mock).

### 6.3 Adding an e2e test
- TBD — see §3 Phase 3 (drill-completion critical flow, driven by `/10x-e2e`).

### 6.4 Adding a test for a new API endpoint
- TBD — see §3 Phase 2 (persistence) and Phase 4 (ownership / IDOR negative case) for the sessions endpoint pattern.
...
- **Auth flows (sign-in / sign-up)** — Supabase stock email-password + OAuth; a
  ...
  login/signup *flow*, NOT authorization on the sessions endpoint — that is
...
- **A full AI-native / vision test layer** — no dedicated AI-native rollout
  ...
  review of the single staff screen is retained as an *optional* supplement
