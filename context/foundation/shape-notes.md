...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "missing capability + engagement gap — no simple, focused tool exists"
    - topic: "insight"
      decision: "the gap is simplicity and focus — dead-simple flashcard drills, not another full music app"
    - topic: "primary persona"
      decision: "the child is primary; parent is secondary (progress tracking)"
    - topic: "auth model"
      decision: "simple login (email + password or OAuth); single shared account for parent and child"
    - topic: "MVP scope-down"
      decision: "cut note duration exercise (type 3) from v1; ship with 2 pitch-recognition exercise types"
    - topic: "timeline"
      decision: "3 weeks of after-hours work"
    - topic: "product type"
      decision: "web-app (PWA)"
    - topic: "scale"
      decision: "small — family use, single-digit users; domain rule is per-user, scale-independent"
    - topic: "timing"
...
- **Then** the child sees exercises one by one (randomly selected from 2 types, weighted toward recent mistakes), gets visual feedback after each answer, and after the last exercise the parent can finish the session and see stats
