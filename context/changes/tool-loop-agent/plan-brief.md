...
Turn `packages/core-reviewer/`'s single-file `runReview()` wrapper into a
...
`packages/core-reviewer/` is a fully standalone package (own `package.json`/
...
`@nuteczki/core-reviewer` exports the reusable `runReview()` agent, the Zod review
...
| 4. Housekeeping                | Name/docs aligned to `core-reviewer`; cruft removed           | Missed stale `code-reviewer` reference                          |

**Prerequisites:** `npm install --prefix packages/core-reviewer` works; auth
...
- `npm run typecheck`, `npm run build`, and the extended offline `npm run smoke`
  ...
  building blocks.
- The package name and README are consistent with the `core-reviewer` folder and
